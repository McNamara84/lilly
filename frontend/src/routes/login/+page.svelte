<script lang="ts">
	...
	let oauthError = $derived(oauthErrorMessage(oauthErrorCode));
	...
	async function handleOAuth(provider: OAuthProvider) {
		if (!authOptions?.oauth[provider]) return;
		oauthLoading = provider;
		errorMessage = '';
		try {
			const authorizationUrl = await startOAuth(provider, 'login');
	...
	function oauthErrorMessage(code: string | null): string {
		switch (code) {
			case 'OAUTH_PROVIDER_DENIED':
				return 'Die Anmeldung beim Anbieter wurde abgebrochen.';
			case 'OAUTH_VERIFIED_EMAIL_REQUIRED':
				return 'Der Anbieter hat keine bestätigte primäre E-Mail-Adresse bereitgestellt.';
			case 'OAUTH_REGISTRATION_REQUIRED':
				return 'Für diese Anbieter-Identität existiert noch kein LILLY-Konto.';
			case 'OAUTH_STATE_INVALID':
				return 'Der Anmeldevorgang ist abgelaufen oder ungültig. Bitte versuche es erneut.';
			case 'OAUTH_PROVIDER_DISABLED':
				return 'Dieser Anmeldeanbieter ist derzeit nicht verfügbar.';
			case 'OAUTH_PROVIDER_ERROR':
				return 'Der Anmeldeanbieter konnte nicht erreicht werden. Bitte versuche es erneut.';
			default:
				return '';
		}
	}
	...
	<div class="glass-elevated w-full max-w-[420px] p-8">
		...
		<form onsubmit={handleSubmit} class="space-y-4" novalidate data-testid="login-form">
			...
			{#if oauthError}
				...
				>
					{oauthError}
					{#if oauthErrorCode === 'OAUTH_REGISTRATION_REQUIRED'}
						<a class="ml-1 underline" href={resolve('/register')}>Jetzt registrieren</a>
					{/if}
				</div>
			{/if}

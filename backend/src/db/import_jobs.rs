...
const IMPORT_JOB_COLUMNS: &str =
    "id, series_id, adapter_name, source_key, trigger_type, scheduled_for, status, \
...
pub async fn is_cancel_requested(pool: &MySqlPool, job_id: u32) -> Result<bool, sqlx::Error> {
    let row: Option<(Option<chrono::NaiveDateTime>, String)> = sqlx::query_as(
        "SELECT cancel_requested_at, status FROM import_jobs WHERE id = ?",
    )
    .bind(job_id)
    .fetch_optional(pool)
    .await?;
    Ok(row.is_none_or(|(requested_at, status)| {
        requested_at.is_some() || status == "cancelled"
    }))
}
...
pub async fn count_import_errors(pool: &MySqlPool, job_id: u32) -> Result<u32, sqlx::Error> {
    let row: (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM import_job_errors WHERE job_id = ?")
            .bind(job_id)
            .fetch_one(pool)
            .await?;
...
mod tests {
    ...
    async fn import_job_lifecycle_is_persistent_and_race_safe_against_mariadb() {
        ...
        .unwrap()
        .unwrap();
        assert!(request_import_cancellation(&pool, pending_job).await.unwrap());
        ...
        assert!(request_import_cancellation(&pool, running_job).await.unwrap());
        ...
        let first_creation = create_import_job_if_idle(
            &pool,
            series_id,
            "test-adapter",
            "test-wiki",
        ...
        let second_creation = create_import_job_if_idle(
            &pool,
            series_id,
            "test-adapter",
            "test-wiki",
        ...
        assert_eq!(concurrent_jobs.iter().filter(|job| job.is_some()).count(), 1);
